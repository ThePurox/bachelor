...
double calcExtPower(int step, int printStep){
	...
	for(int x=0;x<N;x++){
		...
		for(int y=0;y<Ny;y++){
			for(int space=0;space<N_space;space++)
				sum+=sumCurrent(x,y,space)*extForce[IndNN(x,y)][space][(step+1)%2];
			if(VextTimeDependent)
				sum1+=calcdtVext(x,y,step*dt)*density[(step+1)%3][IndNN(x,y)];
		}
	}
	extPower[0][printStep]=step*dt;
	extPower[1][printStep]=sum;
	extPower[2][printStep]=sum1;
...
void calcDiss(int step,int printStep){
	if(printStep!=0 && printStep!=printNumb){
		...
		dissipation[1][printStep]=power[4][printStep]-freeEnergy[2][printStep]+extPower[1][printStep]-extPower[2][printStep];//-(freeEnergy[3][printStep]-freeEnergy[3][printStep-2])/(dt*steps/printNumb*2.);
	...
	for(int i=0;i<steps;i++){
		...
		if(i%int(ceil(steps*1./printNumb))==0) {
			...
			if(printCurrentBool){
				...
				cout<<"Residuum Current: "<<residuumCurrent(i)<<endl;
			}
			if(printPowerBool)
