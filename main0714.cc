	...
	for(int i=0;i<N;i++){
		...
		if(N_space==2){
			for(int j=0;j<N;j++){
				...
				switch(forceInt){
					case -1:
						extForce[IndNN(i,j)][0][count%2]=M_PI/L*sin(2*M_PI*y/L);
			...
			switch(forceInt){
				...
				case 2:
					...
					break;	
				case 3:
					extForce[IndNN(i,0)][0][count%2]=extForceFactor*2*M_PI/L*sin(2*M_PI*(2*x/L+speedRPS/1.*t/tFinal));
					break;
				case 4:
					x0=x-L/4.-(speedRPS*t/tFinal-int(speedRPS*t/tFinal))*L;
					extForce[IndNN(i,0)][0][count%2]=0;
				...
				case 5:
					...
					for(int k=-gaussForceScan;k<=gaussForceScan;k++){
						xx=x0+k*L;
						extForce[IndNN(i,0)][0][count%2]+=-2*extForceFactor*xx/stdev*exp((-xx*xx)/stdev);
					}
					break;
			...
			if(N_space==2){
				for(int n=-over;n<=over;n++){
					for(int j=0;j<N;j++){
						...
						Vext[IndNN(i,j)]+=0;//-extPotFactor0*exp(-(x*x+y*y)/sigVext0);
					}
				}
			}else{
				Vext[IndNN(i,0)]+=0.5*cos(2*M_PI*(2*x/L));//-extPotFactor0*exp(-(x)*(x)/sigVext0);
			}
		}
	}
